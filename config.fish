...
set colour_text_path 97FEED
set colour_text_dirty 1E3050
set colour_text_clean 1E3050

set colour_path 071952
set colour_dirty FF8080
set colour_clean 00DFA2
...
function prompt_git -d "Display the current git state"
    ...
    if command git rev-parse --is-inside-work-tree >/dev/null 2>&1
        ...
        # Check the state of the repo and display the appropriate color
        if [ "$dirty" = "$__fish_git_prompt_char_dirtystate" ]
            prompt_segment $colour_dirty $colour_text_dirty "$branch $dirty"
        else if [ "$dirty" = "$__fish_git_prompt_char_cleanstate" ]
